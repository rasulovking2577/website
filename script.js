...
(function () {
  ...
  if (!isTouch) {
    document.querySelectorAll('.code-window, .price-box, .service-card, .case-card, .why-item').forEach(function (card) {
  ...
  /* ---------- subtle 3D tilt on service & case cards ---------- */
  if (!isTouch && !reduceMotion) {
    document.querySelectorAll('.service-card, .case-card').forEach(function (card) {
      var max = 5; // degrees
      card.addEventListener('mousemove', function (e) {
        var rect = card.getBoundingClientRect();
        ...
        card.style.transform =
          'perspective(700px) rotateX(' + (-py * max) + 'deg) rotateY(' + (px * max) + 'deg) translateY(-4px)';
      });
